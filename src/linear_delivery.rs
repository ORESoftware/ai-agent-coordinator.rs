...
impl LinearDeliveryWorker {
    ...
    pub async fn deliver_job(&self, job: &Job) -> Result<LinearDeliveryReport, LinearDeliveryError> {
        ...
        for directive in &envelope.directives {
            ...
            if self.ledger.is_succeeded(&key).map_err(LinearDeliveryError::internal)? {
    ...
    async fn deliver_directive(
        ...
        if self.config.dry_run {
            return Ok(LinearDirectiveReport {
                issue_identifier: directive.issue_identifier.clone(),
                commit_id: directive.commit_id.clone(),
        ...
        self.wait_for_org_slot(&job.org).await;
        ...
        let attachment_title = format!("Commit {short_commit} in {}/{}", job.org, job.repo);
        let attachment_subtitle = format!(
            "{} {} on {}",
...
fn mutation_key(job: &Job, directive: &LinearDirectiveInput) -> String {
    let action = if directive.closes_issue { "close" } else { "reference" };
    format!(
        "linear:{}:{}:{}:{}:{}:{}",
        job.org,
        job.repo,
...
fn required_string(
    ...
    value
        ...
        .map(str::to_owned)
        .ok_or_else(|| LinearDeliveryError::retryable(format!("Linear response is missing {field}"), Duration::from_secs(30)))
}
...
mod tests {
    ...
    use super::*;
    use crate::jobs::{JobStatus, LinearDirectiveReport as _};
    ...
    async fn mock_server(
        ...
        tokio::spawn(async move {
            axum::serve(
                listener,
                Router::new().route("/graphql", post(mock_graphql)).with_state(state),
            )
            .await
            .unwrap();
        });
        (Url::parse(&format!("http://{address}/graphql")).unwrap(), requests)
    }
    ...
    #[tokio::test]
    async fn dry_run_is_default_safe_and_makes_no_requests() {
        ...
        .unwrap();
        let report = worker.deliver_job(&job(false)).await.unwrap();
    ...
    async fn creates_reference_comment_and_is_idempotent() {
        let (url, requests) = mock_server(vec![
            (StatusCode::OK, issue_response("github.com/sonus-auris", "started", vec![])),
            (StatusCode::OK, json!({"data": {"attachmentCreate": {"success": true, "attachment": {"id": "attachment", "url": "https://github.com/example"}}}})),
            (StatusCode::OK, json!({"data": {"commentCreate": {"success": true, "comment": {"id": "comment"}}}})),
        ]).await;
        let temp = TempDir::new().unwrap();
        let worker = LinearDeliveryWorker::new(
    ...
    async fn closing_directive_updates_configured_state() {
        let (url, requests) = mock_server(vec![
            ...
            (StatusCode::OK, json!({"data": {"attachmentCreate": {"success": true, "attachment": {"id": "attachment", "url": "https://github.com/example"}}}})),
            (StatusCode::OK, json!({"data": {"commentCreate": {"success": true, "comment": {"id": "comment"}}}})),
            (StatusCode::OK, json!({"data": {"issueUpdate": {"success": true, "issue": {"id": "issue-uuid", "identifier": "DEN-455", "state": {"id": "completed-sonus", "name": "Done", "type": "completed"}}}})),
        ]).await;
        let temp = TempDir::new().unwrap();
        let worker = LinearDeliveryWorker::new(
            LinearDeliveryConfig::test(url, false),
        ...
        assert_eq!(requests[3]["variables"]["input"]["stateId"], "completed-sonus");
    }
    ...
    #[tokio::test]
    async fn retries_rate_limits_and_rejects_oversized_responses() {
        let (url, requests) = mock_server(vec![
            (StatusCode::TOO_MANY_REQUESTS, json!({"errors": [{"message": "rate"}]})),
            (StatusCode::OK, issue_response("github.com/sonus-auris", "started", vec![])),
            (StatusCode::OK, json!({"data": {"attachmentCreate": {"success": true, "attachment": {"id": "attachment", "url": "https://github.com/example"}}}})),
            (StatusCode::OK, json!({"data": {"commentCreate": {"success": true, "comment": {"id": "comment"}}}})),
        ]).await;
