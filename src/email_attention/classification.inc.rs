fn classify_message(
    ...
    let urgent_keyword = contains_any(
        &subject_text,
        &snippet_text,
        &["urgent", "asap", "immediate", "action required", "today", "overdue"],
    );
    ...
    let (reason, confidence) = if explicit_deadline_urgent {
        ("An explicit message deadline is due within 24 hours or is overdue.", 0.99)
    } else if security_or_account {
        ("The connector classified this as a security, account, billing, legal, or incident item requiring review.", 0.97)
    } else if message.importance == MessageImportance::High {
        ("The source connector marked this message as high importance.", 0.94)
    } else if urgent_keyword {
        ("The subject or connector-provided snippet contains time-sensitive language.", 0.88)
    } else if message.user_is_next_responder {
        ("Thread state indicates that you are the next expected responder.", 0.93)
    } else if message.direct_request {
        ("The connector detected a direct request addressed to you.", 0.91)
        ...
        ("The message is an active customer, partner, hiring, application, funding, calendar, or travel follow-up.", 0.84)
    } else {
...
fn validate_connector_response(
    ...
    {
        bail!("connector returned an invalid cursor for source {}", source.id);
    }
    ...
    for message in &response.messages {
        ...
        if !stable_ids.insert(message.stable_id.clone()) {
            bail!("connector returned duplicate stable_id for source {}", source.id);
        }
        ...
        {
            bail!("connector returned an invalid snippet for source {}", source.id);
        }
        ...
        {
            bail!("connector returned invalid categories for source {}", source.id);
        }
        if message.material_version.as_ref().is_some_and(|version| {
            ...
            bail!("connector returned invalid material_version for source {}", source.id);
        }
    }
    Ok(())
}
...
fn reject_oversized_content_length(response: &reqwest::Response, max_bytes: usize) -> Result<()> {
    if response
        .headers()
        .get(CONTENT_LENGTH)
...
fn delivery_idempotency_key(run_id: &str, candidates: &[CandidateItem]) -> String {
