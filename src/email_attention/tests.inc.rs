...
mod tests {
    ...
    use super::{
        classify_message, should_emit, AttentionBucket, AttentionStore, ConnectorMessage,
        EmailAttentionAgent, ItemState, ManualEmailAttentionRunRequest, MessageImportance,
        NotificationConfig, Schedule, Settings, SourceConfig, SourceProvider,
    };
    ...
    async fn test_agent() -> (EmailAttentionAgent, TestAdapterState) {
        ...
        let agent = EmailAttentionAgent {
            ...
            store: AttentionStore::open(":memory:").expect("store"),
    ...
    async fn manual_preview_preserves_cursor_and_scheduled_runs_deduplicate_delivery() {
        let (agent, adapter) = test_agent().await;
