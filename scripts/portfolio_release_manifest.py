...
REPOSITORY_STATUSES = {"planned", "verified", "blocked"}
CAPABILITY_STATUSES = {"planned", "verified", "blocked"}
ARTIFACT_STATUSES = {"planned", "verified", "blocked"}
LANE_STATUSES = {"planned", "green", "red", "blocked"}
...
def _nullable_count(value: Any, path: str, errors: list[str]) -> int | None:
    if value is None:
        return None
...
def _github_repo_url(value: Any, path: str, errors: list[str]) -> str | None:
...
def _optional_git_sha(value: Any, path: str, errors: list[str]) -> str | None:
    if value is None:
        return None
    text = _text(value, path, errors, 40, 40)
    if text is not None and not GIT_SHA.fullmatch(text):
        errors.append(f"{path} must be null or a lowercase 40-character Git SHA")
    return text


def _optional_sha256(value: Any, path: str, errors: list[str]) -> str | None:
    if value is None:
        return None
    text = _text(value, path, errors, 64, 64)
    if text is not None and not SHA256.fullmatch(text):
        errors.append(f"{path} must be null or a lowercase SHA-256 digest")
    return text
...
def validate(value: Mapping[str, Any], mode: str = "closure") -> ManifestReport:
    ...
    for index, raw in enumerate(repositories):
        ...
        item = _object(raw, REPOSITORY_KEYS, path, errors)
        ...
        url = _github_repo_url(item.get("github_url"), f"{path}.github_url", errors)
        if url:
        ...
        if status not in REPOSITORY_STATUSES:
            ...
            repository_statuses[repository_id] = status
        head = _optional_git_sha(item.get("head_sha"), f"{path}.head_sha", errors)
        branch = _text(item.get("default_branch"), f"{path}.default_branch", errors, 1, 200)
        ...
        if linear_anchor and not LINEAR.fullmatch(linear_anchor):
    ...
    if (
        ...
        and observed_count + unresolved_count != expected_count
    ):
        errors.append(
    ...
    lanes = _array(root.get("lanes"), "lanes", errors, 1)
    lane_ids: set[str] = set()
    lane_kinds: dict[str, list[dict[str, Any]]] = {}
    for index, raw in enumerate(lanes):
        path = f"lanes[{index}]"
        item = _object(raw, LANE_KEYS, path, errors)
        ...
        if kind:
            lane_kinds.setdefault(kind, []).append(item)
        status = item.get("status")
        if status not in LANE_STATUSES:
            errors.append(f"{path}.status is unsupported")
        head = _optional_git_sha(item.get("head_sha"), f"{path}.head_sha", errors)
        evidence = _optional_sha256(
            item.get("evidence_sha256"), f"{path}.evidence_sha256", errors
        )
    ...
    capabilities = _array(root.get("capabilities"), "capabilities", errors, 1)
    capability_ids: set[str] = set()
    capability_items: list[dict[str, Any]] = []
    for index, raw in enumerate(capabilities):
        path = f"capabilities[{index}]"
        item = _object(raw, CAPABILITY_KEYS, path, errors)
    ...
    artifacts = _array(root.get("artifacts"), "artifacts", errors, 1)
    artifact_ids: set[str] = set()
    artifact_items: list[dict[str, Any]] = []
    for index, raw in enumerate(artifacts):
        path = f"artifacts[{index}]"
        item = _object(raw, ARTIFACT_KEYS, path, errors)
        ...
        if status not in ARTIFACT_STATUSES:
            errors.append(f"{path}.status is unsupported")
        sha = _optional_sha256(item.get("sha256"), f"{path}.sha256", errors)
        provenance = _optional_sha256(
    ...
    if decision_value == "go":
        premature_reasons: list[str] = []
        if not catalog_complete:
            premature_reasons.append("catalog is incomplete")
        if blockers:
            premature_reasons.append("blockers remain")
        if any(item.get("status") != "verified" for item in repositories):
            premature_reasons.append("repositories are not all verified")
        if any(item.get("status") != "verified" for item in capability_items):
            premature_reasons.append("capabilities are not all verified")
        if any(item.get("status") != "verified" for item in artifact_items):
            premature_reasons.append("artifacts are not all verified")
        if any(item.get("status") != "green" for item in lanes):
            premature_reasons.append("lanes are not all green")
        if premature_reasons:
            errors.append(
                "decision.value cannot be go: " + ", ".join(premature_reasons)
            )
    ...
    if not catalog_complete:
        closure_ready = False
        closure_errors.append("catalog_complete must be true")
    if None in (expected_count, observed_count, unresolved_count):
        closure_ready = False
        closure_errors.append("coverage counts must be known")
    elif unresolved_count != 0 or expected_count != observed_count:
        closure_ready = False
        closure_errors.append("repository coverage must be complete")
    for index, item in enumerate(repositories):
        if (
            ...
            closure_ready = False
            closure_errors.append(f"repository {index} is not releasable")
    for index, item in enumerate(capability_items):
        ...
        if (
            item.get("status") != "verified"
            or not isinstance(item.get("maturity"), int)
            or item.get("maturity", 0) < 4
            ...
            closure_ready = False
            closure_errors.append(f"capability {index} lacks independent L4 evidence")
    for index, item in enumerate(artifact_items):
        if (
            ...
            closure_ready = False
            closure_errors.append(f"artifact {index} is not verified")
    missing_kinds = REQUIRED_LANE_KINDS - set(lane_kinds)
    if missing_kinds:
        closure_ready = False
    ...
    for kind in REQUIRED_LANE_KINDS:
        candidates = lane_kinds.get(kind, [])
        if not any(
            ...
            closure_ready = False
            closure_errors.append(f"required lane kind is not certified: {kind}")
    if blockers:
        closure_ready = False
        closure_errors.append("manifest blockers must be empty")
    if decision_value != "go" or not approved_by:
        closure_ready = False
        closure_errors.append("decision must be approved go")
    ...
    if mode == "closure" and not closure_ready:
        errors.extend(f"closure: {message}" for message in closure_errors)
