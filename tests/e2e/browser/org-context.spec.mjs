...
for (const pilot of pilots) {
  ...
  test(`${pilot.owner} machine context, agent, and manifest agree`, async ({
    ...
    expect(context.git_conflict_resolution.mode).toBe("semantic_conceptual_merge");
    expect(context.git_conflict_resolution.history_lookback_commits).toMatchObject({
      minimum: 3,
      maximum: 10,
      inspect_both_sides: true,
      inspect_merge_base: true,
    });
    expect(context.git_conflict_resolution.forbidden_shortcuts).toEqual(
      expect.arrayContaining(["wholesale_ours", "wholesale_theirs"]),
    );
    ...
    expect(agent).toContain("Semantic Git conflict resolution");
