...
/// TypeSpec/JSON Schema peers, reject mutable GitHub Actions dependencies,
...
/// apply bounded peer-authority runtime configuration checks, enforce runtime
/// configuration authority and Rust application lockfile discipline, and harden
...
pub fn audit_repository(options: &RepositoryAuditOptions) -> CommandReport {
    ...
    let report = runtime_toml_sidecar::augment_sidecar_runtime_toml_audit(options, report);
    let mut report = runtime_config_authority::augment_runtime_config_authority_audit(options, report);
