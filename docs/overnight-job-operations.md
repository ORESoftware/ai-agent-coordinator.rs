...
“Reconciliation complete” means every actionable item is classified and owned. “All work complete” is stricter: only `complete` and `already_landed` count as finished. Reports must show both states.
...
HTTP 202, `queued`, a successful fixture-validation job, or an assistant statement is not completion evidence.
...
- Missing activation variables, endpoint, token, worker, source coverage, terminal receipt, or report fails red.
...
6. A manual canary returns a terminal complete receipt with real source coverage.
