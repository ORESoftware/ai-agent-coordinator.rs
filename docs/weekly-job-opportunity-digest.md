...
The scheduler rejects credential, identity-number, payment, MFA, private-key,
and similar sensitive fields. Dry-run output replaces the entire candidate
profile with a redaction marker; it never prints the profile or bearer token.
